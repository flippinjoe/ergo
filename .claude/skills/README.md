...
Repeatable workflows. Each skill is a folder with a `SKILL.md` (short: trigger +
steps) plus heavier reference files loaded **only when the steps say so**. Open
the one that matches your task — don't read them all.

| Skill | Use it when… |
|-------|--------------|
| [`build-and-run`](build-and-run/SKILL.md) | You need to generate the project, build, and launch the macOS app. |
